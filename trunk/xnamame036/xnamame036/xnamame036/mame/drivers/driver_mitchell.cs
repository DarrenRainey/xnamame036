...
namespace xnamame036.mame.drivers
{
    class machine_driver_pang : Mame.MachineDriver
    {
        ...
        eeprom.EEPROM_interface eeprom_interface = new eeprom.EEPROM_interface(
            6,		/* address bits */
    16,		/* data bits */
    "0110",	/*  read command */
    "0101",	/* write command */
    "0111"	/* erase command */
        ...
        {
            cpu.Add(new Mame.MachineCPU(Mame.CPU_Z80, 8000000, driver_pang.readmem, driver_pang.writemem, driver_pang.readport, driver_pang.writeport, Mame.interrupt, 2));
        ...
        public override void nvram_handler(object file, int read_or_write)
        {
            if (read_or_write != 0)
            {
                ...
                if (nvram_size[0] != 0)	/* Super Pang, Block Block */
                    Mame.osd_fwrite(file, nvram, nvram_size[0]);	/* NVRAM */
            }
            else
            {
                eeprom.EEPROM_init(eeprom_interface);
                ...
                if (file != null)
                {
                    ...
                    if (nvram_size[0] != 0)	/* Super Pang, Block Block */
                        Mame.osd_fread(file, nvram, nvram_size[0]);	/* NVRAM */
                }
                else
        ...
        static void get_bg_tile_info(int col, int row)
        {
            ...
            byte attr = pang_colorram[tile_index];
        ...
        public override int vh_start()
        {
            ...
            pang_objram = new _BytePtr(pang_videoram_size[0]);
            Array.Clear(pang_objram.buffer, pang_objram.offset, pang_videoram_size[0]);
        ...
        static void draw_sprites(Mame.osd_bitmap bitmap)
        {
            ...
            for (int offs = 0x1000 - 0x40; offs >= 0; offs -= 0x20)
            {
                ...
                int color = attr & 0x0f;
                int sx = pang_objram[offs + 3] + ((attr & 0x10) << 4);
                int sy = ((pang_objram[offs + 2] + 8) & 0xff) - 8;
                code += (attr & 0xe0) << 3;
                if (flipscreen != 0)
                {
                    sx = 496 - sx;
    ...
    class driver_pang : Mame.GameDriver
    {
        ...
        {
            if (machine_driver_pang.pang_videoram[offset] != data)
            {
                machine_driver_pang.pang_videoram[offset] = (byte)data;
                Mame.tilemap_mark_tile_dirty(machine_driver_pang.bg_tilemap, (offset / 2) % 64, (offset / 2) / 64);
            }
        }
        static int mgakuen_videoram_r(int offset)
        {
            return machine_driver_pang.pang_videoram[offset];
        }
        static void mgakuen_objram_w(int offset, int data)
        {
            machine_driver_pang.pang_objram[offset] = (byte)data;
        }
        static int mgakuen_objram_r(int offset)
        {
            return machine_driver_pang.pang_objram[offset];
        }
        static void pang_videoram_w(int offset, int data)
        {
            if (machine_driver_pang.video_bank != 0) mgakuen_objram_w(offset, data);
            else mgakuen_videoram_w(offset, data);
        }
        static int pang_videoram_r(int offset)
        {
            if (machine_driver_pang.video_bank != 0) return mgakuen_objram_r(offset);
            else return mgakuen_videoram_r(offset);
        }
        static void pang_colorram_w(int offset, int data)
        {
            if (machine_driver_pang.pang_colorram[offset] != data)
            {
                machine_driver_pang.pang_colorram[offset] = (byte)data;
                Mame.tilemap_mark_tile_dirty(machine_driver_pang.bg_tilemap, offset % 64, offset / 64);
            }
        }
        ...
        {
            return machine_driver_pang.pang_colorram[offset];
        }
        static void pang_gfxctrl_w(int offset, int data)
        {
            ...
            if (machine_driver_pang.flipscreen != (data & 0x04))
            {
                machine_driver_pang.flipscreen = data & 0x04;
                Mame.tilemap_set_flip(Mame.ALL_TILEMAPS, machine_driver_pang.flipscreen != 0 ? (Mame.TILEMAP_FLIPY | Mame.TILEMAP_FLIPX) : 0);
            }
        ...
        public static Mame.MemoryWriteAddress[] writemem =
{
	...
	new Mame.MemoryWriteAddress( 0xc800, 0xcfff, pang_colorram_w, machine_driver_pang.pang_colorram ),
	new Mame.MemoryWriteAddress( 0xd000, 0xdfff, pang_videoram_w, machine_driver_pang.pang_videoram, machine_driver_pang.pang_videoram_size ),
        ...
        public static Mame.GfxDecodeInfo[] gfxdecodeinfo =
{
	...
	new Mame.GfxDecodeInfo(  Mame.REGION_GFX2, 0, spritelayout,   0,  16 ), /* colors 0- 255 */
        ...
        static void pang_video_bank_w(int offset, int data)
        {
            ...
            machine_driver_pang.video_bank = data;
        }
