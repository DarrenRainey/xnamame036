...
namespace xnamame036.mame
{
    class FM
    {
...
#if FM_STEREO_MIX
        const byte YM2151_NUMBUF =1;
        const byte YM2608_NUMBUF =1;
        const byte YM2610_NUMBUF =1;
        const byte YM2612_NUMBUF =1;
#else
        ...
        public delegate void FM_TIMERHANDLER(int n, int c, int cnt, double stepTime);
        public delegate void FM_IRQHANDLER(int n, int iq);
        public delegate void _EG(FM_SLOT SLOT);
        ...
        const int TL_SHIFT = (TL_BITS + 1 - (14 - 16));
        const int FM_OUTSB = (TL_SHIFT - FM_OUTPUT_BIT);
        const int FM_MAXOUT = ((1 << (TL_SHIFT - 1)) - 1);
        const int FM_MINOUT = (-(1 << (TL_SHIFT - 1)));

        const int EG_AST = 0;			/* start of Attack phase */
        const int EG_AED = (EG_ENT << ENV_BITS);	/* end   of Attack phase */
        const int EG_DST = EG_AED;		/* start of Decay/Sustain/Release phase */
        const int EG_DED = (EG_DST + ((EG_ENT - 1) << ENV_BITS));	/* end   of Decay/Sustain/Release phase */
        const int EG_OFF = EG_DED;			/* off */
#if FM_SEG_SUPPORT
...
const int  EG_UED   ((3*EG_ENT)<<ENV_BITS)  /* end of SEG UPSISE */
        ...
        //const int  OPN_CHAN(N) (N&3)
        //const int  OPN_SLOT(N) ((N>>2)&3)
        //const int  OPM_CHAN(N) (N&7)
        //const int  OPM_SLOT(N) ((N>>3)&3)
        /* slot number */
        const int SLOT1 = 0;
        const int SLOT2 = 2;
        const int SLOT3 = 1;
        ...
        const int OUTD_RIGHT = 1;
        const int OUTD_LEFT = 2;
        const int OUTD_CENTER = 3;

        /* FM timer model */
        const int FM_TIMER_SINGLE = (0);
        const int FM_TIMER_INTERVAL = (1);

        class FM_3SLOT
        {
            public uint[] fc = new uint[3];
            public byte[] fn_h = new byte[3];
        ...
        class FM_OPN
        {
            ...
            public FM_CH[] P_CH;
            public uint[] FN_TABLE = new uint[2048];
#if FM_LFO_SUPPORT
            /* LFO */
            public uint LFOCnt;
            public uint LFOIncr;
        ...
        public class FM_SLOT
        {
            ...
            /* Envelope Generator */
            public _EG eg_next;	/* pointer of phase handler */
        ...
        class FM_CH
        {
            ...
            public IntSubArray connect1=new IntSubArray(1);		/* pointer of SLOT1 output    */
            public IntSubArray connect2 = new IntSubArray(1);		/* pointer of SLOT2 output    */
            public IntSubArray connect3 = new IntSubArray(1);		/* pointer of SLOT3 output    */
            public IntSubArray connect4 = new IntSubArray(1);		/* pointer of SLOT4 output    */
            /* LFO */
        ...
        class FM_ST
        {
            ...
            public IntSubArray AR_TABLE = new IntSubArray(94);		/* Atttack rate tables */
            public IntSubArray DR_TABLE = new IntSubArray(94);		/* Decay rate tables   */
        ...
        static object cur_chip;
        static FM_ST State;
        static FM_CH[] cch = new FM_CH[8];
#if FM_LFO_SUPPORT
        static uint LFOCnt, LFOIncr;
#endif
        static int[] out_ch = new int[4];
	...
	9*64,10*64,11*64,12*64
        ...
        static int ML(double n) { return (int)(n * 2); }
...
/* DT2=3 *SQL(3)   */
 ...
 SC( 8),SC( 9),SC(10),SC(11),SC(12),SC(13),SC(14),SC(31)
        ...
        const byte TYPE_SSG = 0x01;
        const byte TYPE_OPN = 0x02;
        const byte TYPE_LFOPAN = 0x04;
        const byte TYPE_6CH = 0x08;
        const byte TYPE_DAC = 0x10;
        const byte TYPE_ADPCM = 0x20;
        ...
        static int OPM_CHAN(int n) { return n & 7; }
        static int OPM_SLOT(int n) { return (n >> 3) & 3; }
                ...
                case 0x00: /* 0x00-0x1f */
                    switch (r)
                    {
                        ...
                        case 0x0f:	/* Noise freq (ch7.op4) */
                            /* b7 = Noise enable */
                            /* b0-4 noise freq  */
                            OPM.NoiseIncr = (v & 0x80) == 0 ? 0u :
                                /* !!!!! unknown noise freqency rate !!!!! */
                                (uint)((1 << FREQ_BITS) / 65536 * (v & 0x1f) * OPM.ST.freqbase);
                            cur_chip = null;
#if true
                            if ((v & 0x80) != 0)
                                Mame.printf("OPM Noise mode selelted\n");
...
/* FD=3 */
  ...
  8 , 8, 9,10,11,12,13,14,16,17,19,20,22,22,22,22
        ...
        static byte[] OPM_DTTABLE = OPN_DTTABLE;
        ...
        class YM2151
        {
            ...
            public Mame.mem_write_handler PortWrite;/*  callback when write CT0/CT1 */
        }



        static YM2151[] FMOPM = null;
...
#endif
        public static int OP_OUT(int PG, uint EG)
        {
            return SIN_TABLE[(PG / (0x1000000 / SIN_ENT)) & (SIN_ENT - 1)][(int)EG];
        }
        static int OP_OUTN(int PG, uint EG)
        {
            return NOISE_TABLE[(int)((PG / (0x1000000 / SIN_ENT)) & (SIN_ENT - 1))][(int)EG];
        }
        static void OPM_CALC_CH7(FM_CH CH)
        {
            ...
            if (NoiseIncr != 0)
            {
                ...
                if (eg_out4 < EG_CUT_OFF)
                    CH.connect4[0] += OP_OUTN((int)NoiseCnt, eg_out4);
            }
            else
            {
                if (eg_out4 < EG_CUT_OFF)
...
/* ---------- calcrate timer A ---------- */
void INTERNAL_TIMER_A(ST,CSM_CH)					
{													
...
{														
	if( ST.TBC && (ST.Timer_Handler==0) )				
        ...
        static YM2203[] FM2203 = null;
        static int YM2203NumChips;
        ...
        static void OPNWriteReg(FM_OPN OPN, int r, int v)
{
	...
	case 0xb0:
        ...
        public static int SSGRead(int n) { return ay8910.AY8910Read(n); }
        ...
        public static void SSGWrite(int n,int a, int v){ay8910.AY8910Write(n,a,v);}
        ...
        public static void OPMSetPortHandler(int n,Mame.mem_write_handler PortWrite )
{
